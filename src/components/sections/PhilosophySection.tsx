...
import { DESKTOP_MOTION, useMotionSettings } from "@/components/animation/motion";
  ...
  useGSAP(
    () => {
      ...
      media.add(DESKTOP_MOTION, () => {
        gsap.set(cards, { autoAlpha: 0, y: 24, scale: 0.985, rotation: 0, zIndex: 0, pointerEvents: "none" });
        gsap.set(cards[0], { autoAlpha: 1, y: 0, scale: 1, zIndex: 1 });
        ...
        for (let index = 1; index < cards.length; index += 1) {
          ...
          timeline
            .set(cards[index - 1], { autoAlpha: 0, y: -18, scale: 0.985, zIndex: 0 }, position)
            .set(cards[index], { autoAlpha: 1, y: 0, scale: 1, rotation: 0, zIndex: index + 1 }, position)
            .to(indexes[index - 1], { opacity: 0.28, x: 0, duration: 0.06 }, position)
            .to(indexes[index], { opacity: 1, x: 8, duration: 0.08, ease: "power2.out" }, position);
        }
      });
      return () => media.revert();
    },
    { scope: section, dependencies: [desktop], revertOnUpdate: true },
  );

  return (
    <section ref={section} id="process" className={`relative overflow-x-clip bg-[#030506] ${desktop ? "min-h-[185svh]" : "py-20"}`}>
      <div className={desktop ? "relative sticky top-0 flex min-h-svh items-center overflow-hidden pb-8 pt-24" : "relative"}>
        ...
        <div className="container relative z-10">
          <div className={`build-heading border-b border-white/10 pb-5 ${desktop ? "hidden" : ""}`}>
          ...
          <div className={`mt-8 gap-4 ${desktop ? "hidden" : "grid"}`}>
            {steps.map((step, index) => (
              <article key={step.principle} className="rounded-2xl border border-white/10 bg-white/[.035] p-6">
                <div className="flex items-center justify-between font-mono text-xs uppercase tracking-[.13em] text-cyan-200"><span>0{index + 1}</span><span>{step.principle}</span></div>
          ...
          <div className={`min-h-[560px] grid-cols-[.72fr_1.28fr] items-center gap-12 ${desktop ? "grid" : "hidden"}`}>
            ...
            <div className="relative h-[500px] [perspective:1400px] [transform-style:preserve-3d] 2xl:h-[560px]">
              {steps.map((step, index) => (
                <article key={step.principle} className="build-card absolute inset-0 isolate flex flex-col justify-between overflow-hidden rounded-[2rem] border border-white/14 bg-[#081013] p-8 shadow-[0_45px_120px_rgba(0,0,0,.58)] will-change-[transform,opacity] [backface-visibility:hidden] 2xl:p-14">
                  ...
                  <div className="relative flex items-center gap-4 border-t border-white/10 pt-5 font-mono text-xs uppercase tracking-[.13em] text-white/32">
