...
export function Header(){
 ...
 useEffect(()=>{document.body.classList.toggle("menu-open",open);return()=>document.body.classList.remove("menu-open")},[open]);
  return <><header className={`fixed inset-x-0 top-0 z-50 transition-all duration-500 ${scrolled?"border-b border-white/10 bg-black/65 backdrop-blur-xl":""}`}><div className="container flex h-20 items-center justify-between"><a href="#home" aria-label="Gholzad Home" className="flex items-center transition-transform duration-300 hover:scale-105"><Image src="/images/logo/logo-full.png" alt="Gholzad Logo" width={180} height={48} className="h-10 w-auto object-contain filter drop-shadow-[0_0_10px_rgba(255,255,255,0.25)]" priority /></a><nav aria-label="Primary" className="hidden items-center gap-7 lg:flex">{nav.slice(0,-1).map(([l,id])=><a className="text-xs text-white/60 transition hover:text-white" href={`#${id}`} key={id}>{l}</a>)}<a className="pill !min-h-10 text-xs" href="#contact">Let&apos;s talk</a></nav><button aria-label={open?"Close menu":"Open menu"} aria-expanded={open} className="grid size-11 place-items-center rounded-full border border-white/20 lg:hidden" onClick={()=>setOpen(!open)}>{open?<X/>:<Menu/>}</button></div><div className="absolute inset-x-0 bottom-0 h-px bg-white/10"><motion.div aria-hidden className="h-full origin-left bg-cyan-300" style={{scaleX:scrollYProgress}}/></div></header>
