...
export function Loader() {
  ...
  useEffect(() => {
    ...
    const timeout = window.setTimeout(
      () => setDone(true),
      reduceMotion ? 150 : mobile ? 450 : 650,
    );
  ...
  return (
    <AnimatePresence>
      {!done && (
        ...
        >
          <motion.div aria-hidden className="absolute inset-y-0 left-0 w-[50.1%] origin-left bg-[#030506]" exit={reduceMotion ? { opacity: 0 } : { x: "-102%" }} transition={{ duration: 0.5, ease: [0.76, 0, 0.24, 1] }} />
          <motion.div aria-hidden className="absolute inset-y-0 right-0 w-[50.1%] origin-right bg-[#030506]" exit={reduceMotion ? { opacity: 0 } : { x: "102%" }} transition={{ duration: 0.5, ease: [0.76, 0, 0.24, 1] }} />
          <div
            aria-hidden
